...
class RgbConfigWidget(AmkWidget):
...
class RgbConfig(BasicEditor):
    def __init__(self, layout_editor):
        ...
        self.keyboardWidget = RgbConfigWidget(layout_editor, self)
    ...
    def rebuild(self, device):
        ...
        if self.valid():
            ...
            self.keyboardWidget.set_keys(self.keyboard.amk_rgb[0]["leds"], [])
            self.keyboardWidget.setEnabled(True)
            self.reset_keyboard_widget()

    def valid(self):
        return isinstance(self.device, VialKeyboard) and \
               (self.device.keyboard and len(self.device.keyboard.amk_rgb) > 0)
    ...
    def reset_keyboard_widget(self):
        if self.valid():
            ...
            for widget in self.keyboardWidget.widgets:
                widget.masked = True
                led = self.keyboard.get_rgb_matrix_led_by_index(widget.desc.col)
    ...
    def on_key_clicked(self):
        ...
        led = self.keyboard.get_rgb_matrix_led_by_index(key.desc.col)
        if led is None:
            return
    ...
    def on_color_selected(self):
        ...
        for idx, key in self.keyboardWidget.active_keys.items():
            index = key.desc.col
            led = self.keyboard.get_rgb_matrix_led_by_index(index)
            if led is not None:
                led.set_hue(hue)
                led.set_sat(sat)
                led.set_val(val)
