...
class MacroRecorder(BasicEditor):
    ...
    def __init__(self):
        ...
        buttons.addStretch()
        self.btn_save = QPushButton(tr("MacroRecorder", "Save/保存"))
        self.btn_save.clicked.connect(self.on_save)
        btn_revert = QPushButton(tr("MacroRecorder", "Revert/恢复"))
    ...
    def on_change(self):
        ...
        data = self.serialize()
        memory = len(data)
        self.lbl_memory.setText("Memory used by macros/宏占用的存储空间: {}/{}".format(memory, self.keyboard.macro_memory))
